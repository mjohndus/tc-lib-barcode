...
class Dynamic
{
    ...
    protected $latchTable;

    protected $MODE_UPPER = 0;
    protected $MODE_LOWER = 1;
    protected $MODE_DIGIT = 2;
    ...
    protected function getShift($fromMode, $toMode)
    {
    ...
    protected function getCharMapping($mode, $char)
    {
    ...
    protected function updateStateListForPair($pairCode)
    {
        $result = [];
        foreach ($this->states as $state) {
            $state->endBinaryShift();
    ...
    protected function shiftAndAppend($token, $mode, $value)
    {
        $token = clone $token;
    ...
    protected function latchAndAppend($token, $mode, $value)
    {
        $token = clone $token;
        $current_mode = $token->getMode();
    ...
    protected function addBinaryShiftChar(&$token)
    {
        $current_mode = $token->getMode();
