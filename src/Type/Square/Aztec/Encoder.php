...
class Encoder
{
    ...
    protected function mSet($x, $y)
    {
        $this->MATRIX[$x][$y] = 1;
    }

    protected function toByte($bstream)
    {
        $data = [];
    ...
    protected function appendBstream(&$bstream, $data, $bits)
    {
        for ($i = $bits - 1; $i >= 0; $i--) {
            $bstream[] = ($data >> $i) & 1;
        }
    ...
    */
    public function encode($content, $eccPercent, $hint)
    {
        ...
        switch ($hint) {
            ...
            case "binary":
                ...
                break;
            default:
                throw new BarcodeException('Unknown encoder');
        }
        ...
        if ($layers == $LAYERS_FULL) {
                        throw new BarcodeException('Data too large');
        }
    ...
    protected function drawBullsEye($center, $size)
    {
        for ($i = 0; $i < $size; $i += 2) {
    ...
    protected function generateCheckWords($stuffedBits, $totalSymbolBits, $wordSize)
    {
        $messageSizeInWords = intval((count($stuffedBits) + $wordSize - 1) / $wordSize);
        ...
        if ($messageSizeInWords == 0) {
                        throw new BarcodeException('No data bytes provided');
        }

        if ($ecBytes == 0) {
                        throw new BarcodeException('No error correction bytes');
        }
    ...
    protected function bitsToWords($stuffedBits, $wordSize, $totalWords)
    {
